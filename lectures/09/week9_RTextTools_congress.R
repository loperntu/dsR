...
# TRAIN ON THE "text" and "cong" COLUMNS
congress_matrix <- create_matrix(cbind(USCongress["text"],USCongress["cong"]), 
                                 language ="english", 
                                 removeNumbers = TRUE, 
                                 stemWords = TRUE, 
...
corpus <- create_container(congress_matrix,
                           USCongress$major,
                           trainSize = 1:4000, 
                           testSize = 4001:4449, 
                           virgin = FALSE)
...
models <- train_models(corpus, 
                       algorithms = c("SVM","MAXENT")) #Make take some time.
